...
class CustomAuthManager {
  ...
  Future signOut() async {
    ...
    aqaumaticAppAuthUserSubject.add(
      AqaumaticAppAuthUser(loggedIn: false),
    );
    persistAuthData();
  }

  Future<AqaumaticAppAuthUser?> signIn({
    String? authenticationToken,
    String? refreshToken,
    DateTime? tokenExpiration,
    String? authUid,
  ...
  AqaumaticAppAuthUser? _updateCurrentUser({
    String? authenticationToken,
    String? refreshToken,
    DateTime? tokenExpiration,
    String? authUid,
    ...
    final updatedUser = AqaumaticAppAuthUser(
      loggedIn: true,
      uid: authUid,
      userData: userData,
    );
    aqaumaticAppAuthUserSubject.add(updatedUser);
    persistAuthData();
    return updatedUser;
  }
...
AqaumaticAppAuthUser? currentUser;
